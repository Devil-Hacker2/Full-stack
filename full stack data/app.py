...
db=mysql.connect(
    ...
    user='root',
    password='root',
    database='newdemodb' #The database is newdemodb and the table name is users and the attributes are Name,roll_no and password
)

cur=db.cursor()

app=Flask(__name__)
app.secret_key='veera'
...
def collectFromRegister():
    ...
    cur.execute('SELECT roll_no FROM users')
    result=cur.fetchall()
    flag=0
    for x in result:
        if(rollno==x[0]):
    ...
    if(flag==0):
        sql="INSERT INTO users (roll_no,name,password) VALUES (%s,%s,%s)"
